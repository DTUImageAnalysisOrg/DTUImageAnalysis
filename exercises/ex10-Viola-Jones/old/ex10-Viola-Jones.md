...
??? EXAMPLE "Code Template for exercises 1-3"
...
**Question 1**: *Which functions does what? Where is the detection actually happening?*
...
We'll first investigate how the model performs on a simple image of a stop sign. In the **data** folder, the image *stop_sign.jpg* can be found. First, try to visualize the image. We may evaluate the model performance qualitatively using the *VJ_performance_eval.py* script and running the following command in your terminal:
...
**Question 6**: *Looking at some of the computed haar features, can you deduce which features the model attends to at the different classification stages?*
...
**Question 7**: *How does the model perform? When does it correctly detect or not detect the stop sign?*
...
**Question 8**: *Examine the script file and try to modify the scale_factor parameter of the detection function. Explain the effects of increasing and decreasing the scale factor (accuracy vs. frames per second). Try and see if you can find a scale-factor for which the stop sign is detected at all distances while it is completely contained in the frame.*
...
We'd now like to take a deeper look at what a Viola-Jones style model actually bases decisions on in relation to an image window. In the **haar_features**-directory in data you are given sub-directories, each showcasing 3 or 2 Haar features used at a speicifc classifier stage. 


### Exercise 16: Finding the corresponding slice
We will have a look at an example Haar feature provided in **stage_3/11.png** (the feature image). The background image is **annotation_img.jpg** (the reference image). 

Notice that the reference image has shape $24\times24$, whereas the feature image is upscaled to be $240\times240$ per default by opencv for visualization. We now want to map the coordinates for the Haar feature in the feature image to the corresopnding coordinates in the reference image. 

This can be done using that the white area has intensity 255 and the black regions have intensity 0 (you can verify this by using a histogram). As there are other areas (the text) which have the same values, we slice those away before finding the slice: 

```py 
offset_y = 50 
feature_im = stage_img[offset_y:,:] #crop away upper text region
ys, xs = np.where((feature_im == 0) | (feature_im == 255))
...
Now, your task is to crop the reference image such that we include the region of the Haar feature, but also the boundary using one pixel. Remember to load the image as ubyte and grayscale. 
...
??? TIP "Hint:" 
    The functions numpy.cumsum or cv2.integral may be of use.

??? INFO
    *It is important that the boundary of your integral image goes beyond the ROI by at least 1 pixel in all directions. cv2.integral fixes this for you by zero-padding the boundary, but keep this in mind when using numpy.cumsum.*
...
## Part 4: Haar feature diagnostics.old 
---
Viola-Jones is based on Haar features. Each stage of the decision tree evaluates one or multiple Haar features, and applies a threshold for "continuing" the cascade. 
...
### Exercise 16: Loading and cropping images (Optional)
...
### Exercise 17: Compute the integral image of image slice
...
### Exercise 18: Compute the Haar feature
Now, compute your Haar feature. To do so, extract the haar corners of each Haar region and compute their individual Haar sum. Finally, compute:
...
### Exercise 19: Trying to relate a Haar feature to a single scale class instance 
...
![[figures/haar_feature_response.png]]
...
### Exercise 22: Train an object detector (**Optional**)
...
We advise you to use stop-signs for the positive class and car-side for the negative class, but you are of course free to use other classes as well. 
...
??? TIP "Training parameters for a good stop-sign detector"
    We've had luck with: 
    ```opencv_traincascade -data stop_sign_detector -vec positive.vec -bg  stop_sign_neg.dat -numPos 58  -numNeg 64 -numStages 18 -w 24 -h 24```
...
2. Run: `python VJ_performance_eval.py --model <path/to/my_detector/cascade.xml> --input your_image.jpg --type image --w <width> --h <height>`
